...
//        MyAsyncTask(this@ClientActivity).execute("http://172.30.1.4:10381")

        btn_client_send.setOnClickListener {
            val email:String = edit_client_id.text.toString()
            val name = edit_client_pw.text.toString()

            sendAsyncTask().execute("http://172.30.1.4:10381/elections/post/", email, name)
        }

        btn_client_get.setOnClickListener {
            getAsyncTask(this@ClientActivity).execute("http://172.30.1.4:10381/elections/getusers/", null, null)
        }
